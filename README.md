...
- Enable **Minimum wheel step** to set the minimum point distance generated by
  ...
  notch. Use **Step multiplier** presets from `0.5×` to `3×` to scale the
  effective minimum without changing the saved Step or amplifying movements
  already above the threshold. Explicit modifier keys can still reduce or
  increase the result. The Step range remains `0.01–100.00 pt`, its default is
  `18.00 pt`, and the multiplier defaults to `1×`. Turn the feature off to
  disable both controls while retaining their saved values. When both axes are
  present, the effective minimum scales the complete movement vector without
